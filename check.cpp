#include "check.h"



const unsigned char ucCRC16H[] =
{   };
const unsigned char ucCRC16L[] =
{  };

Check::Check() {

}

// ================ 函数功能：查表计算CRC16校验 ==============
bool Check::crc16(const char *dat, int len) {
    unsigned char ucTmpCRC16H = 0xFF;
    unsigned char ucTmpCRC16L = 0xFF;
    unsigned char ucIndex;
    while (len--) {
        ucIndex = ucTmpCRC16H ^ *dat++;
        ucTmpCRC16H = ucTmpCRC16L ^ ucCRC16H[ucIndex];
        ucTmpCRC16L = ucCRC16L[ucIndex];
    }
    if ((ucTmpCRC16H == 0) && (ucTmpCRC16H == 0)) return true;
    else return false;
}


bool Check::crc16(const QByteArray &dat) {
    unsigned char ucTmpCRC16H = 0xFF;
    unsigned char ucTmpCRC16L = 0xFF;
    unsigned char ucIndex;

    foreach(unsigned char x, dat) {
        ucIndex = ucTmpCRC16H ^ x;
        ucTmpCRC16H = ucTmpCRC16L ^ ucCRC16H[ucIndex];
        ucTmpCRC16L = ucCRC16L[ucIndex];
    }
//    for(int i=0;i<dat.size();i++)
//    {
//        ucIndex = ucTmpCRC16H ^ (quint8)dat[i];
//        ucTmpCRC16H = ucTmpCRC16L ^ ucCRC16H[ucIndex];
//        ucTmpCRC16L = ucCRC16L[ucIndex];
//    }
    if ((ucTmpCRC16H == 0) && (ucTmpCRC16H == 0)) return true;
    else return false;
}

void Check::crc16Append(char *dat, int len) {
    unsigned char ucTmpCRC16H = 0xFF;
    unsigned char ucTmpCRC16L = 0xFF;
    unsigned char ucIndex;
    while (len--) {
        ucIndex = ucTmpCRC16H ^ *dat++;
        ucTmpCRC16H = ucTmpCRC16L ^ ucCRC16H[ucIndex];
        ucTmpCRC16L = ucCRC16L[ucIndex];
    }
    *dat++ = ucTmpCRC16H;
    *dat = ucTmpCRC16L;
}

void Check::crc16Append(QByteArray &dat) {
    unsigned char ucTmpCRC16H = 0xFF;
    unsigned char ucTmpCRC16L = 0xFF;
    unsigned char ucIndex;

    foreach(unsigned char x, dat) {
        ucIndex = ucTmpCRC16H ^ x;
        ucTmpCRC16H = ucTmpCRC16L ^ ucCRC16H[ucIndex];
        ucTmpCRC16L = ucCRC16L[ucIndex];
    }
    dat.append(ucTmpCRC16H);
    dat.append(ucTmpCRC16L);
}

bool Check::sum8(const QByteArray &dat) {

    if (dat.size() < 2) return false;
    qint8 i, sum = 0;
    int size = dat.size();
    for (i = 0; i < size - 1; i++) {
        sum += dat.at(i);
    }

    if (sum == dat.at(i)) return true;
    else return false;
}

void Check::sum8Append(QByteArray &dat) {
    quint8 sum = 0;
    foreach(quint8 x, dat) {
        sum += x;
    }
    dat.append(sum);
}




